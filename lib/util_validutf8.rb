            ...
            begin
                if new.encoding == Encoding::UTF_8
                    ...
                    # this is how we like our cookies
                        ...
                        # hack valid utf-8
                        new.encode!(Encoding::UTF_16, :invalid=>:replace,
                                    :undef=>:replace, :replace=>replace)
                    ...
                    begin
                        if new.valid_encoding?
                            ...
                            conv = Encoding::Converter.new(old.encoding, 
                                                       Encoding::UTF_8,
                                                       :invalid => :replace,
                                                       :undef   => :replace,
                                                       :replace => replace)
                            while !old.empty?
                                case conv.primitive_convert(old,new, nil, nil,
                                                         {:partial_input=>true})
                                # ruby sucks
                                # valid_encoding? returns false positives
                            ...
                            # UTF8_KDDI, UTF8_SOFTBANK, UTF8_DOCOMO
                            # will produce invalid utf8
                            ...
                            if Encoding::Converter::BUGGY_CONVERTER[new.encoding]
                                ...
                                new.encode!(new.encoding, :invalid => :replace,
                                                          :undef   => :replace, 
                                                          :replace=>rep)
                            end
                            # real transcode
                            new.encode!(Encoding::UTF_8, new.encoding,
                                        :invalid => :replace,
                                        :undef   => :replace,
                                        :replace => replace)
                            # As I said, ruby sucks.
                            # It seems valid_encoding? is lazy, its result (may)
                            # not get updated after converting a string. We need
                            # to force it to do work for us.
                            new.force_encoding(Encoding::UTF_8)
                            if !new.valid_encoding?
                        ...
                        # now let's hope the encoding is somewhat ascii-like...
...
if false
  time = 0
  bytes = 0
  String.send(:define_method,:valid_utf8,EbuStl::Util.method(:valid_utf8))
